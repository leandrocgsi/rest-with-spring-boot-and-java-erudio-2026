...
- `Dockerfile` (`eclipse-temurin:25-jdk`) is the plain `COPY target/*.jar app.jar` + `ENTRYPOINT ["java","-jar","/app.jar"]`, since the PDF export works from the fat jar (see Runtime setup). `.dockerignore` keeps only `target/*.jar` in the build context, so run `mvn clean package` first.
...
- The API can run with `docker compose up -d --build` or with `java -jar`. Port 8080 is often taken on the developer's machine: publish another port and pass `--env-var baseUrl=http://localhost:<port>` to newman.
