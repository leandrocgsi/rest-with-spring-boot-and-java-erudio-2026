...
| Arquivo | O que é |
|---|---|
| `Formação Spring Boot 2026 - ... .postman_collection.json` | 48 requisições (Auth, person, book, file, e-Mail), cada uma com testes (`pm.test`) |
| `SPRING_BOOT_JAVA_ERUDIO.postman_environment.json` | Variáveis usadas pela collection |
| `files/` | Arquivos de exemplo usados nos uploads (`sample.txt`, `people.csv`, `people.xlsx`) |

## Usando no Postman

1. Importe a collection e o environment e selecione o environment `SPRING_BOOT_JAVA_ERUDIO`.
...
npx newman run Collections/*.postman_collection.json \
  -e Collections/SPRING_BOOT_JAVA_ERUDIO.postman_environment.json \
